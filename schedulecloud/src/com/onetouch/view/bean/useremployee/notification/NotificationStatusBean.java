...
@ManagedBean(name="notificationStatusBean")
@RequestScoped
				...
				if(DateUtil.compareDateAndTime(schedule.getExpiretime(), new Date())<0){
					...
					if(schedule!=null && schedule.getSchedulestatus()==1){
						if(status.equalsIgnoreCase("accept")){
							...
							getScheduleService().updateScheduleStatus(scheduleId,employeeId,companyId,eventPosId,3);
							notificationStatus = "Schedule Declined";
							notificationDesc = "You have declined an offer to work for the following event. Admin will be notified accordingly , you dont need to respond further";
						}
					...
					}else if(schedule!=null && schedule.getSchedulestatus()==3){
						notificationStatus = "Already Declined";
						notificationDesc = "You have previously declined the schedule, for any changes please contact the administrator";
					}else{

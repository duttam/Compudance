...
@ManagedBean(name="offerAcceptBean")
@RequestScoped
public class OfferAcceptBean extends BaseBean{
	...
	public void init(){
		if(scheduleId!=null&&employeeId!=null&&companyId!=null&&eventPosId!=null&&status!=null){
			schedule = getScheduleService().getOfferSchedule(scheduleId,eventPosId,employeeId,companyId);
			if(schedule!=null && schedule.isActive()){
				boolean offerAccept = getScheduleService().saveOfferedSchedule(schedule,2);
				if(DateUtil.compareDateAndTime(schedule.getExpiretime(), new Date())<0){
					notificationStatus = "Schedule Expired";
					notificationDesc = "The specified  offer has expired. You didn't respond in time.";
				}
				else{
					if(schedule!=null && schedule.getSchedulestatus()==1){
						...
						if(offerAccept){
							...
							notificationStatus = "Schedule Expired";
							notificationDesc = "The specified  offer has expired. The position has been filled.";

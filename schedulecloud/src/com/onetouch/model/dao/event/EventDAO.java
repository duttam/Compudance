...
import com.onetouch.model.mapper.event.EventWithEmployeeScheduleExtractor;
...
public class EventDAO extends BaseDAO{
	...
	private final String selectSalesPersonEvent = " select e.*,e1.id as creator_id, e1.firstname as creator_firstname,e1.lastname as creator_lastname,et.id as eventtype_id, et.name eventtype_name " +
			...
			"left join event_type et on et.id = e.event_type WHERE e.location_id IN ( " +
			"SELECT sl.loc_id from salesperson_loc sl where sl.emp_id = ? and sl.company_id = ? " +
			"union " +
			"select l.id from location l where l.location_type = 'custom' and l.company_id = ? and l.region_id = ?) and e.region_id = ? "; 
	private  String selectEventPositionByIdsql=" SELECT e.*, ep.id as eventPosId,ep.position_id,ep.reqd_number, ep.displayOrder, date_format(ep.start_time,'%l:%i %p') as starttime, ep.notes as evntPosNotes ," +
												...
												"emp1.id as adminId,emp1.firstname as adminFirstname, emp1.lastname as adminLastname, emp1.homephone as adminHomephone, " +
												"emp2.id as managerId,emp2.firstname as managerFirstname, emp2.lastname as managerLastname, " +
												"emp3.id as salesPersonId, emp3.firstname as salesPersonFirstname, emp3.lastname as salesPersonlastname, emp3.homephone as salesHomephone, " +
												"et.id as eventtype_id,et.name as eventtype_name, " +
												"l.id as locId,l.name as locName,l.code,l.address1,l.address2,l.city,l.state,l.zipcode,l.email,l.contact_name,l.location_type " +
	...
	private final String updateEventSql = "update event set name=?,owner_id=?,coordinator_id=?,event_startdate=?,event_enddate=?,start_time=?,end_time=?,dress_code=?,notes=?,description=?,host=?,location_id=?,timezone=?,guest_count = ?,event_type = ?,salesPersonNote = ?,created_by = ? where id=? and company_id=? and region_id=?";
	private final String eventDetailSql = "select e.*, emp1.id as owner_id, emp1.firstname as owner_firstname,emp1.lastname as owner_lastname, " +
											"emp2.id as coordniator_id,emp2.firstname as coordinator_firstname, emp2.lastname as coordinator_lastname, " +
											"loc.id as loc_id, loc.name as loc_name, loc.code as loc_code,loc.address1 as loc_address1, loc.city as loc_city, loc.statecode as loc_statecode, " +
											"loc.state as loc_statename, loc.contact_name as loc_contactname,loc.zipcode as zipcode,loc.email as email,loc.parking_direction " +
											"from event e, employee emp1, employee emp2, location loc " +
											"where e.owner_id = emp1.id and e.coordinator_id = emp2.id and e.location_id = loc.id and e.id = ? and e.company_id = ?";
	...
	public List<Event> findAll(final Integer companyId,final Region region, CustomUser user){
		...
		if(user.isAdmin()){
			...
			return getJdbcTemplate().query(selectSalesPersonEvent+orderByClause,new Object[]{user.getEmp_id(),companyId,companyId,region.getId(),region.getId()}, new EventHomeRowMapper()); 
		else
	...
	public void update(final Event event){
		getJdbcTemplate().update(updateEventSql, new PreparedStatementSetter() {
			...
			public void setValues(PreparedStatement ps) throws SQLException {
				...
				ps.setString(16, event.getSalesPersonNotes());
				ps.setInt(17,event.getCreatedBy());
				ps.setInt(18,event.getId());
				ps.setInt(19,event.getTenant().getId());
				ps.setInt(20,event.getEventRegion().getId());
	...
	public List<Event> findAllByDateRangeAndLocation(Integer companyId, Date start, Date end, CustomUser customUser, Location selectedLocation, final Region currentRegion) {
		...
		if(selectedLocation!=null && selectedLocation.getId()>0)
			where = " and l.id = ?";
		String eventorderClause = " order by e.event_startdate,e.start_time asc ";
		 sql = "SELECT distinct e.*, ep.id as eventPosId,ep.position_id,ep.reqd_number, ep.displayOrder, ep.notes as evntPosNotes , " +
                            "date_format(ep.start_time,'%l:%i %p') as starttime,  " +
                            "date_format(ep.end_time,'%l:%i %p') as endtime, pos.name as position_name, " +
                            "DATE_FORMAT(ep.start_date, '%m/%d/%Y') as start_date, DATE_FORMAT(ep.end_date, '%m/%d/%Y') as end_date, " +
                            "emp1.id as adminId,emp1.firstname as adminFirstname, emp1.lastname as adminLastname,emp1.homephone as adminHomephone, " +
                            "emp2.id as managerId,emp2.firstname as managerFirstname, emp2.lastname as managerLastname, " +
                            "emp3.id as salesPersonId, emp3.firstname as salesPersonFirstname, emp3.lastname as salesPersonlastname, emp3.homephone as salesHomephone," +
                            "et.id as eventtype_id,et.name as eventtype_name, " +
                    ...
                    " from event e " +
                      "left join event_position ep on ep.event_id = e.id " +
                      ...
                      "left join employee emp3 on emp3.id = e.created_by " +
                      "left join event_type et on et.id = e.event_type " +
                    "where e.status = 'PUBLISHED' and (e.event_startdate BETWEEN DATE(?) AND DATE(?)) and e.company_id = ? and e.region_id = ? ";
			
                if(customUser.isAdmin()){
		        
			if(selectedLocation!=null && selectedLocation.getId()>0)
				return getJdbcTemplate().query(sql + where+eventorderClause,new Object[]{s,e,companyId,currentRegion.getId(),selectedLocation.getId()}, new EventWithPositionExtractor());
			else
				return getJdbcTemplate().query(sql+eventorderClause ,new Object[]{s,e,companyId,currentRegion.getId()}, new EventWithPositionExtractor());
			
		} else if(customUser.isManager()) {
                        roleCondition = " and    e.coordinator_id = ? ";
			if(selectedLocation!=null && selectedLocation.getId()>0)
				return getJdbcTemplate().query(sql + roleCondition + where+eventorderClause,new Object[]{s,e,companyId,currentRegion.getId(),selectedLocation.getId(),customUser.getEmp_id()}, new EventWithPositionExtractor());
			else
				return getJdbcTemplate().query(sql + roleCondition+eventorderClause ,new Object[]{s,e,companyId,currentRegion.getId(),customUser.getEmp_id()}, new EventWithPositionExtractor());
		}
		else if(customUser.isSalesPerson()){
                        //roleCondition = " and e.location_id IN ( SELECT sl.loc_id from salesperson_loc sl where sl.emp_id = ? and sl.company_id = ?) ";
                        roleCondition = " and e.location_id IN ( SELECT sl.loc_id from salesperson_loc sl where sl.emp_id = ? and sl.company_id = ? union select l.id from location l where l.location_type = 'custom' and l.company_id = ? and l.region_id = ?) ";
			...
			if(selectedLocation!=null && selectedLocation.getId()>0)
				return getJdbcTemplate().query(sql + roleCondition + " and e.location_id = ? "+eventorderClause,new Object[]{s,e,companyId,currentRegion.getId(),customUser.getEmp_id(),companyId,companyId,currentRegion.getId(),selectedLocation.getId()}, new EventWithPositionExtractor());
			else
				return getJdbcTemplate().query(sql + roleCondition+eventorderClause, new Object[]{s,e,companyId,currentRegion.getId(),customUser.getEmp_id(),companyId,companyId,currentRegion.getId()}, new EventWithPositionExtractor());			
		} else
			return new ArrayList<Event>();
	...
	public List<EventType> findAllEventTypes(final Tenant tenant) {
		List<EventType> eventtypes = getJdbcTemplate().query("select * from event_type where company_id = ?", new Object[]{tenant.getId()}, new RowMapper<EventType>() {
	...
	public void updateEventType(final EventType eventType) {
		getJdbcTemplate().update("update event_type set name = ?, description = ? where  id =? and company_id = ?", new PreparedStatementSetter() {
			
			@Override
			public void setValues(PreparedStatement ps) throws SQLException {
				ps.setString(1, eventType.getName());
				ps.setString(2, eventType.getDescription());
				ps.setInt(3,eventType.getId());
				ps.setInt(4,eventType.getTenant().getId());
			}
		});
		
	}

	public List<Event> findAllSchedulesByEmployee(Date rosterStartDate,
			Date rosterEndDate, Employee employee, Integer tenant_id, Integer region_id) {
		String findScheduleByEmployee = " SELECT ep.schId,ep.scheduleStatus,e.*, " +
				"ep.id as eventPosId,ep.position_id,ep.reqd_number, ep.displayOrder,ep.notes as evntPosNotes ,ep.position_name, " +
				"date_format(ep.start_time,'%l:%i %p') as starttime, " +
				"date_format(ep.end_time,'%l:%i %p') as endtime, " +
				"DATE_FORMAT(ep.start_date, '%m/%d/%Y') as start_date, " +
				"DATE_FORMAT(ep.end_date, '%m/%d/%Y') as end_date,  " +
				"emp1.id as adminId,emp1.firstname as adminFirstname, emp1.lastname as adminLastname, emp1.homephone as adminHomephone, " +
				"emp2.id as managerId,emp2.firstname as managerFirstname, emp2.lastname as managerLastname, " +
				"emp3.id as salesPersonId, emp3.firstname as salesPersonFirstname, emp3.lastname as salesPersonlastname, emp3.homephone as salesHomephone, " +
				"et.id as eventtype_id,et.name as eventtype_name, " +
				"l.id as locId,l.name as locName,l.code,l.address1,l.address2,l.city,l.state,l.zipcode,l.email,l.contact_name " +
				"from (select * from event where company_id = ? " +
				"and region_id = ? " +
				"and (event_startdate BETWEEN DATE(?) and DATE(?))) as e " +
				"inner join( " +
				"select epos.*,s.status as scheduleStatus,s.id as schId, p.name as position_name " +
				"from event_position epos, schedule s,position p " +
				"where epos.id = s.event_pos_id " +
				"and epos.position_id = p.id " +
				"and s.employee_id = ? " +
				"and s.company_id = ? " +
				"and s.active=true " +
				"and epos.active=true " +
				"and epos.region_id = ? ) as ep on ep.event_id = e.id " +
				"left join location l on l.id = e.location_id " +
				"left join employee emp1 on emp1.id = e.owner_id " +
				"left join employee emp2 on emp2.id = e.coordinator_id " +
				"left join employee emp3 on emp3.id = e.created_by " +
				"left join event_type et on et.id = e.event_type";  
		return getJdbcTemplate().query(findScheduleByEmployee,new Object[]{tenant_id,region_id,rosterStartDate,rosterEndDate, employee.getId(),tenant_id,region_id} ,new EventWithEmployeeScheduleExtractor(employee));
	}
